...
console.log("wsServerPublicKey", json.parse(env.ensureConf("auth-public-key")));
...
export const createYWebsocketServer = async ({
  ...
  await registerYWebsocketServer(
    ...
    async (req) => {
      ...
      const [, , rawToken] = /(^|,)yauth-(((?!,).)*)/.exec(
        headerWsProtocol,
      ) ?? [null, null, req.getQuery("yauth")];
      if (rawToken == null) {
        throw new Error("Missing Token");
      }
      // URL-decode the token (µWebSockets getQuery doesn't decode automatically)
      const token = decodeURIComponent(rawToken);
      console.log("token", token);
      // verify that the user has a valid token
      console.log("public Key", wsServerPublicKey);
      // verify that the user has a valid token
